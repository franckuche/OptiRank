...
document.addEventListener('DOMContentLoaded', function() {
  // Suppression du log : window.safeLogger.debug('Initialisation des filtres améliorés');
  ...
  function debugSkippedLinks() {
    if (!window.lastResults || !window.lastResults.links) {
      // Suppression du log : window.safeLogger.debug('%cAucun résultat à analyser', 'color: #F29725; font-weight: bold;');
      return;
    }
    ...
    if (skippedLinks.length > 0) {
      // Suppression du log : window.safeLogger.debug('%c=== LIENS IGNORÉS ===', 'background: #492A25; color: white; padding: 5px; font-weight: bold; font-size: 14px;');
      // Suppression du log : window.safeLogger.debugEmoji("", "Total des liens ignorés: ${skippedLinks.length} sur ${window.lastResults.links.length} liens");
      ...
      if (skippedLinks.length > 0) {
        ...
        // Suppression du log : window.safeLogger.debug('%c=== STATISTIQUES DES LIENS IGNORÉS ===', 'background: #F29725; color: white; padding: 5px; font-weight: bold; font-size: 14px;');
        // Suppression du log : window.safeLogger.debug({
        ...
        // Suppression du log : window.safeLogger.debug('%cAucun lien ignoré trouvé', 'color: #F29725; font-weight: bold;');
      }
    }
  }
  ...
  function syncSelectsWithCheckboxes() {
    // Suppression du log : window.safeLogger.debug('Synchronisation des filtres - Valeurs sélectionnées:', {
    ...
    if (!filterAll || !filterValid || !filterBroken || !filterRedirects) {
      // Suppression du log : window.safeLogger.error('Certaines checkboxes n\'existent pas, impossible de synchroniser les filtres');
      return;
    }
    ...
    // Suppression du log : window.safeLogger.debug('État des checkboxes après synchronisation:', {
    ...
    if (window.lastResults) {
      // Suppression du log : window.safeLogger.debug('Application des filtres sur les résultats');
      window.generateResultsTable(window.lastResults);
    } else {
      // Suppression du log : window.safeLogger.warn('Aucun résultat disponible pour appliquer les filtres');
    }
  }
  
  // Fonction pour appliquer directement les filtres sans passer par les checkboxes
  function applyFiltersDirectly() {
    // Suppression du log : window.safeLogger.debug('Application directe des filtres améliorés');
    ...
    // Suppression du log : window.safeLogger.debug('Résultats disponibles:', {
    ...
    // Suppression du log : window.safeLogger.debug('Filtres actifs:', activeFilters);
    
    // Appliquer le filtre de statut
    if (statusFilter && statusFilter.value !== 'all') {
      // Suppression du log : window.safeLogger.debug('Application du filtre de statut:', statusFilter.value);
      ...
      // Suppression du log : window.safeLogger.debugEmoji("", "Après filtre statut: ${filteredLinks.length} liens restants");
    }
    
    // Appliquer le filtre d'attributs
    if (attributeFilter && attributeFilter.value !== 'all') {
      // Suppression du log : window.safeLogger.debug('Application du filtre d\'attributs:', attributeFilter.value);
      ...
      // Suppression du log : window.safeLogger.debugEmoji("", "Après filtre attributs: ${filteredLinks.length} liens restants");
    }
    
    // Appliquer le filtre de type
    if (typeFilter && typeFilter.value !== 'all') {
      // Suppression du log : window.safeLogger.debug('Application du filtre de type:', typeFilter.value);
      ...
      // Suppression du log : window.safeLogger.debugEmoji("", "Après filtre type: ${filteredLinks.length} liens restants");
    }
    
    // Appliquer le filtre de recherche si nécessaire
    const searchTerm = document.getElementById('search-links')?.value?.toLowerCase() || '';
    if (searchTerm.trim() !== '') {
      // Suppression du log : window.safeLogger.debug('Application du filtre de recherche:', searchTerm);
      ...
      // Suppression du log : window.safeLogger.debugEmoji("", "Après filtre recherche: ${filteredLinks.length} liens restants");
    }
    
    // Afficher les liens filtrés pour débogage
    // Suppression du log : window.safeLogger.debugEmoji("", "Total après filtrage: ${filteredLinks.length} liens");
    if (filteredLinks.length > 0) {
      // Suppression du log : window.safeLogger.debug('Exemple de lien filtré:', filteredLinks[0]);
    }
